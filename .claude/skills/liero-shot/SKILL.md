...
default match**, keyboard-driven, indefinite (no tick bound, no self-check).
Window title `"Liero-rs — default match"`. Since Step 4½c it starts like C++
NEW GAME (`game::new_game`): `Settings::default()`, a level generated by
`sim::levelgen` from a fresh seed (stderr prints `openliero: new game, seed N
on …`), then weapon selection, then the match. F5 and the post-match restart
are the next NEW GAME: a new seed on the level the last match left
(`regenerate_level` is off, as in C++). The committed fixture
