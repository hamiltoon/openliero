...
| **4d** *(✓ DONE — bit-exact, master+components 126 ticks, on PR #3; closes the Slice-3/4 deferrals; ProcessSight omission proven (handgun `laserSight=true`); surfaced dumper `loading_time=0` + an unlisted shell-landing `BlitImageOnMap` deferral)* | the **Slice-3 deferrals** that belong here: dig-body `DrawDirtEffect` (texture 7 carving — first **live** `n_draw_back=true`), reload branch (`ammo<=0 → loading_left`/`ComputedLoadingTime`), `leave_shell_timer` shell-drop (`nobject_types[7]`), `ProcessSight` (laser — **audited inert, stays omitted**), weapon-change `load_change` gate | **handgun** (+ dig, no weapon) | — (reuses `wobjects`/`nobjects`) | + leave-shell `rand(leave_shells)` (4a-guarded→live) + 5-draw shell-expiry burst + 2× dig `rand(2)` | live (dig carve + reload `loading_left`) |
