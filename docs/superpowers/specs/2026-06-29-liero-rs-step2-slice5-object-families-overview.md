...
|---|---|---|---|---|---|
| **5a — Splinters (O9)** | `WObject::BlowUpObject`'s **splinter arm** (`weapon.cpp:96-114`): the `splinter_scatter==0` branch → per splinter `rand(128)`+`rand(2)`+`NObject::Create2`. Replaces the `splinter_amount<=0` tripwire ported in 4c. **+ non-default `loading_time` golden** (4d carry, independent). | **cannon** → `medium_explosion` + 5 splinters (scatter 0 ⇒ Create2). Worms **out of range** ⇒ no `DoDamage`. | — (reuses `nobjects`) | **none** (`weapon`+`[ammo]` already supported) | **none** — free of stats/cycles/viewport traps; first **live** exercise of the (already-ported) blow-away kick via cannon's `affect_by_explosions=true` (hash-neutral, freed-after) |
