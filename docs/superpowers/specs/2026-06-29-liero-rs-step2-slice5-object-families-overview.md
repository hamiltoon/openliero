...
| **5d — Death + respawn** ✅ *MILESTONE GREEN + fuzzed (361 ticks bit-exact; 4-variant respawn fuzz, {2,3,6,7} trial counts)* | death block (`worm.cpp:369-426`: death sound `rand(3)`, blood spray ×kMax, worm-parts spray ×8 (`{7,21,…,105}`, the `i<=105` bound is 8 iters — NOT 7)); pre-death drip (`rand(health+6)`/`rand(3)`/`rand(3)`); `killed_timer` countdown + lives gate; **`BeginRespawn`** (2 `rand` per trial, **live-level-dependent trial count**); `DoRespawning` (`DrawDirtEffect` + raw `rand()&1`). | a worm killed by an explosion, then respawns; **fuzz** the respawn search. | — | extend driver (full worm-loop death path; `quick_sim=false`) | **deepest desync risk** — trial count depends on live level + enemy pos |
...
worm-parts `for i=7;i<=105;i+=14` (**8×** — `{7,21,35,49,63,77,91,105}`; the `i<=105`
bound includes 105, so 8 iterations, not 7) `[rand(14)` + `Create2`-internals`]`.
