//! ASCII visualization of the Slice-5d death->respawn scenario, for human
//! eyeballing (worm1 dying @tick 87, the 150-tick dead countdown, the
//! BeginRespawn teleport @~237, and DoRespawning going live @~304) instead of
//! reading hash columns. Reuses the exact scenario/TC/const setup from
//! `oracle-tests/tests/sim_slice5d_golden.rs` (same input k-1 keying), then
//! renders a handful of "interesting" ticks with [`sim::debug::render_ascii`]
//! to both stdout and `target/frames/slice5d/tick_NNNNN.txt`.
//!
//! Run: `cargo run -p oracle-tests --example render_slice5d`
//!
...
/// Hand-picked ticks straddling the death (~87), the dead countdown (120), the
/// BeginRespawn teleport (~237), and the DoRespawning completion (~304).
const INTERESTING_TICKS: [u32; 9] = [80, 86, 87, 90, 120, 237, 260, 304, 320];

fn main() {
    // --- Parse the scenario (single source of truth, shared with the C++ dumper).
    // Verbatim setup from sim_slice5d_golden.rs's scenario/TC/const block.
    ...
    // --- Build tick-0 state (same `new` signature as 5b/5c). ------------------
    ...
    // --- THE CRITICAL 5d STEP: set the blood consts AND the respawn consts.
    ...
    // --- Output dir: <workspace>/target/frames/slice5d (git-ignored via /target). ---
    let out_dir: PathBuf = [env!("CARGO_MANIFEST_DIR"), "..", "target", "frames", "slice5d"]
        .iter()
        .collect();
    std::fs::create_dir_all(&out_dir).expect("create target/frames/slice5d");
