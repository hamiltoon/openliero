//! ASCII visualization of the Slice-5'a T7 per-pixel nobject (cannon splinter)
//! worm-hit scenario, for human eyeballing (the cannon shell '+' flying to the
//! left wall, the explosion, a splinter '*' flying back east, and the contact
//! tick @146 where it lands on a solid worm pixel and blood sprays) instead of
//! reading hash columns. Reuses the exact scenario/TC/const setup from
//! `oracle-tests/tests/sim_slice5prime_nobj_golden.rs` (same input k-1
//! keying), then renders a handful of "interesting" ticks with
//! [`sim::debug::render_ascii`] to both stdout and
//! `target/frames/slice5prime_nobj/tick_NNNNN.txt`.
//!
//! Run: `cargo run -p oracle-tests --example render_slice5prime_nobj`
//!
...
/// Hand-picked ticks straddling the fire (47), the wall explosion (~122-123),
/// the splinter transit toward the contact tick (146), and the settle-out.
const INTERESTING_TICKS: [u32; 6] = [47, 60, 140, 145, 146, 150];

fn main() {
    // --- Parse the scenario (single source of truth, shared with the C++ dumper).
    // Verbatim setup from sim_slice5prime_nobj_golden.rs's scenario/TC/const block.
    ...
    // --- Set the blood consts (as 5b/5d/5prime). --------------------------------
    ...
    // --- Output dir: <workspace>/target/frames/slice5prime_nobj (git-ignored). --
    let out_dir: PathBuf = [
        env!("CARGO_MANIFEST_DIR"),
        "..",
        "target",
    ...
    .iter()
    .collect();
    std::fs::create_dir_all(&out_dir).expect("create target/frames/slice5prime_nobj");
