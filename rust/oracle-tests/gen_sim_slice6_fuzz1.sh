#!/usr/bin/env bash
# Regenerates golden/sim_slice6_fuzz1.txt from the C++ oracle by driving the SAME
# scenario (golden/sim_slice6_fuzz1_scenario.txt) the Rust difftest reads. Slice-6 T7
# >1000-tick FUZZ variant 1 of 5: 2 worms seeded DEAD (visible 0, pos 0 0,
...
# dumper change — the scenario is the single source of truth). game seed 385, input
...
"build/$PRESET/Release/oracle_dump_sim_physics" \
  "rust/oracle-tests/golden/sim_slice6_fuzz1_scenario.txt" \
  "rust/oracle-tests/golden/sim_slice6_fuzz1.txt"
echo "wrote rust/oracle-tests/golden/sim_slice6_fuzz1.txt"
