#!/usr/bin/env bash
# Regenerates golden/sim_slice6_fuzz5.txt from the C++ oracle by driving the SAME
# scenario (golden/sim_slice6_fuzz5_scenario.txt) the Rust difftest reads. Slice-6 T7
# >1000-tick FUZZ variant 5 of 5: 2 worms seeded DEAD (visible 0, pos 0 0,
...
# dumper change — the scenario is the single source of truth). game seed 43, input
# seed 424242, max_bonuses 4, real arena Levels/modern_test.lev. The reproducible
...
"build/$PRESET/Release/oracle_dump_sim_physics" \
  "rust/oracle-tests/golden/sim_slice6_fuzz5_scenario.txt" \
  "rust/oracle-tests/golden/sim_slice6_fuzz5.txt"
echo "wrote rust/oracle-tests/golden/sim_slice6_fuzz5.txt"
