//! Per-tick differential test for the Slice-5b EXPLOSIVES -> large_explosion -> worm1
//! WOUND -> blood spray -> live `bobjects` blood pool against the C++ oracle — THE
//! MILESTONE of 5b. The golden (`golden/sim_slice5b.txt`, 121 lines for ticks 0..=120)
...
//! worm0 visible + grounded with **EXPLOSIVES** in weapon slot 0, worm1 VISIBLE +
//! grounded + stationary at x=115. worm0 raises the gun and FIRES (input tick 38) the
//! 3 explosives projectiles, which lob flat-LEFT, bounce (`bounce=30`), crawl low-left,
//! ANIMATE (`num_frames=3`) and AIR-BURST (`time_to_explo=55`) at x~94/99/110, each
//! spawning a **`large_explosion`** SObject (id 0). worm1's centre (x=115) sits INSIDE
//! the +/-20 detect box of two of the three blasts, so the per-worm damage loop (T2:
//! `DoDamage` + sobject arm) WOUNDS worm1 (health 100 -> 88 -> 82, stays > 0 — death is
//! a later slice) and sprays `kBloodAmount` **type-6 blood nobjects** [`rand(128)` +
...
//! (x=150, >=40px from every blast centre) is far outside every box and takes NO damage.
//!
//! Golden columns (hashes hex):
//!   `<tick> <state_hash> <rng> <level> <worm0> <worm1> <bob> <bon> <sob> <nob> <wob>`
//!
//! ## The closed-gate re-scope (Controller Option B) — why this lands as a MILESTONE
//!
//! 5b was re-scoped from the dart to EXPLOSIVES, the only CLOSED-GATE, splinter-free
//! weapon, so the ONLY worm-damage path is the sobject explosion AABB — deferring the
//! per-pixel `CheckForSpecWormHit` (the in-flight wobject/nobject worm-hit arms) to a
//! follow-up slice:
//!   * CLOSED GATE: explosives has `hit_damage=0, blow_away=0, blood_on_hit=0,
//!     worm_collide=false` => the wobject in-flight worm-hit gate (`weapon.cpp:290`)
//!     NEVER opens => C++ skips the in-flight hit AND the Rust (which omits it) matches.
//!     No in-flight hit => no per-pixel test, no 16x16 box over-approximation.
//!   * `splinter_amount=0` => NO splinter nobjects => the nobject worm-hit deferral is
//!     never reached.
//!   * The wobject FLIGHT physics explosives needs — `bounce` and the `num_frames`
//!     projectile animation — landed in T4a (commit 2274c24), both RNG-free, so
//!     explosives now FLIES in the Rust sim. The only worm-damage path is the
//!     `large_explosion` per-worm AABB (`sobject.cpp:47-115`), LIVE since T2.
//!
//! ## What a bit-exact match proves — the whole O10 damage+blood+bobjects chain
//!
//! A bit-exact match over 121 ticks proves the entire T2 (sobject worm-damage arm +
//! `DoDamage`) + T3 (blood-trail -> `BObject` -> bobjects driver) port end-to-end vs
//! the C++ oracle, now flying a bouncing/animating closed-gate projectile (T4a). Two
//! RNG facts characterise 5b:
//!   * EXPLOSIVES Fire DOES draw rand (`distribution=16000` spread + `time_to_explo_v`
//!     timeout, per part): `rng` is FLAT until the Fire tick (38), JUMPS there, keeps
//!     moving while the 3 projectiles fly + bounce, then JUMPS again at each air-burst.
//!     worm0 has `recoil=0`, so its column is CONSTANT once grounded.
//!   * At each air-burst the cluster draws: explosion sound `rand(4)`
//!     (`large_explosion` numSounds=4); the per-worm `DoDamage(z)` (no rand) + blow-away
//!     (no rand) + `kBloodAmount` x [`rand(128)` + `Create2`(`rand(40)`+`rand(40000)`x2)]
//!     + the `rand(3)` gate; then the 9x9 dirt-throw; then the crater `DrawDirtEffect`
//!     `rand(2)`. The `rng` keeps moving for several ticks while blood + bobjects drip.
//!
...
/// `draw_dirt_effect`: the large_explosion's `dirt_effect` indexes this bank on the
...
fn sim_slice5b_blood_bobjects_match_cpp_oracle() {
    ...
    // --- Parse the golden vectors (ticks 0..=120, master + 9 components). ------
    ...
    // --- THE 5b HEADLINE: the explosives air-bursts WOUND worm1 -> blood nobjects -> the
    // live `bobjects` blood pool. The golden must therefore carve (`level` moves),
    // spawn an sobject (the large_explosion), spawn nobjects (blood + dirt) AND leave
    ...
    // --- Override weapon slot 0 with EXPLOSIVES, mirroring the C++ dumper's
    // `ResolveWeapon("EXPLOSIVES")`. The name is the *scenario's* `weapon 0` directive
    // and must match `common->weapons[i].name` exactly (UPPERCASE "EXPLOSIVES"). -----
    ...
    // --- Build the two worm inits from the scenario. BOTH worms get EXPLOSIVES in slot
    ...
    // defaults true/100). `blood = 100` is the per-worm `kBloodAmount = blood *
    // power_sum / 100` multiplier the damage arm reads (sobject.cpp:96). ---------
    ...
    // --- Coverage guards read from the DRIVEN SimState (the wound proofs), never
    ...
    let mut saw_large_explosion = false; // sobject id 0 = the explosives' create_on_exp
    ...
    let mut record = |state: &SimState| {
        ...
        if state.sobjects.iter().any(|s| s.id == 0) {
            saw_large_explosion = true;
        }
    ...
    // worm1 wounded-not-killed: health > 0 EVERY tick (survives) AND < 100 from
    // the wound tick onward (the damage actually landed).
    assert!(
        worm1_health_always_positive,
        "worm1 health must stay > 0 every tick (wounded, NOT killed — death is a later slice)"
    );
    ...
    // No self-damage: worm0 health == 100 EVERY tick (it fired but stayed clear of
    // every blast's +/-detect_range box).
    assert!(
    ...
    // The explosives' create_on_exp is the large_explosion sobject (id 0), and the
    // per-worm blood arm spawns type-6 blood nobjects.
    assert!(
        saw_large_explosion,
        "the spawned sobject must be large_explosion (id == 0, the explosives' create_on_exp)"
    );
    assert!(
    ...
    // Terrain genuinely carved (the craters); no bonuses this slice.
    assert!(
        level_seen.len() >= 2,
