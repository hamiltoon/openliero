...
// then drives them N ticks by calling the UNMODIFIED `worm->Process(game)` for each
// worm in `game.worms` order and dumps one hash record per tick (tick 0 plus one
// after each of the N Process passes => N+1 lines). Each line:
...
//
// Why drive `worm->Process` directly and NOT `Game::ProcessFrame`: ProcessFrame
// draws RNG every tick (the bonus-drop roll) and increments `cycles` — both are
// Slice-6 ProcessFrame-integration concerns, not physics. Under empty input and
// full health every RNG-drawing / pool-spawning branch of `Process` is skipped, so
// `rand.last` stays 0 (the `rng` column is a constant 0), `cycles` stays 0, and the
// level is never dug — the run is honest, isolated worm physics. The dumper must
// NOT call ProcessFrame, ++cycles, or GenerateFromSettings.
//
